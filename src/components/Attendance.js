  ...
  return (
    <div>
      ...
      <div className="flex justify-between mb-2">
        <h1 className="text-sm text-gray-600 font-bold ml-1">Attendance Calendar</h1>
        <h1 className="text-sm font-bold text-green-500 mr-1">View all</h1>
      </div>
      ...
      <div className="custom-scrollbar h-[300px] overflow-y-scroll">
        {AttendenceData.map((entry, index) => (
          <div
            key={index}
            className="dark:bg-black bg-white border border-gray-700 rounded-lg shadow-md pt-2 pb-3 px-3 flex items-center justify-between mb-2"
          >
            <div className="">
              <h1 className="text-xl dark:text-white text-black font-bold">{entry.date}</h1>
              <p className="text-xs text-gray-500">{entry.month}</p>
            </div>
            <div className="ml-5 mb-1">
              <h2 className="text-xs pt-2 dark:text-white text-black font-semibold">{entry.hours}</h2>
              <p className="text-xs font-bold text-gray-600">{entry.timeRange}</p>
